...
从类别维度看，模型在 \(\cls{0}\) 和 \(\cls{1}\) 上表现最稳定，recall 分别为 0.9600 和 0.9800，F1-score 分别为 0.9412 和 0.9515。这说明尺度校准后的 UNI2-h 表征可以较好地区分一部分形态边界清晰的类别。相对地，\(\cls{3}\) 和 \(\cls{4}\) 的 recall 均为 0.7600，F1-score 分别为 0.8085 和 0.7755，是最终模型的主要薄弱环节。由于每折每类验证样本约为 10 张，这些数值更适合作为类别间难度差异和错误方向的证据，而不宜被外推为稳定的总体估计。
