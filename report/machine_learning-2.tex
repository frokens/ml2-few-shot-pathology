...
本次期末任务要求在每类仅 50 张训练样本的条件下构建 5 分类图像分类器，并在大规模、类别不均衡的测试集上提交预测结果。训练集共 250 张 \(32\times32\) RGB 图像，类别为 \texttt{Class\_0} 到 \texttt{Class\_4}；测试集共 61,881 张无标签图像。评价指标采用 macro-F1 与 balanced accuracy，因此除总体判别能力外，还需要关注各类别召回率与精度之间的均衡性。

本文将该任务归纳为一个总体问题和三个技术子问题。总体问题是：如何在极少有标签样本下迁移强视觉表征，并使其适应大规模不均衡测试分布。三个子问题分别是：第一，\(32\times32\) 单细胞图像与 \(224\times224\) 病理基础模型输入视野之间存在尺度错配；第二，250 张样本不足以支撑全量微调，需要采用参数高效适配；第三，测试集虽无标签但规模很大，引入伪标签时需要控制高置信易类对伪标签池的主导效应。由此，本文关注的研究缺口并非普通 5 分类 CNN 的重新训练，而是如何将大尺寸病理 tile 上预训练的基础模型迁移到低分辨率、中心细胞级别、标注极少的分类任务。
...
图 \ref{fig:dataset-counts} 展示训练集的类别分布。训练集在标签数量上完全均衡，每类 50 张，但样本量极小；测试集则有 61,881 张无标签图像，规模约为训练集的 247.5 倍。这个比例说明测试集不仅是最终评价集合，也包含大量无标签分布信息；在标签不可见的设定下，测试集信息只能通过模型预测概率、置信度排序和软标签等无监督信号进入训练。
...
图 \ref{fig:contact-sheet} 展示不同类别的放大样本。每张原图只有 \(32\times32\) 像素，放大后可以看到两个特点：第一，类别差异主要来自细胞核形态、染色深浅、胞质边界和局部纹理，而不是大尺度物体轮廓；第二，同一类别内部也存在明显颜色和形态变化。例如 \(\cls{3}\) 的训练图平均亮度最高，\(\cls{4}\) 的平均局部对比度最高（见表 \ref{tab:train-image-stats}），说明颜色统计本身不足以稳定定义类别边界。因此，本研究采用具有病理形态先验的基础模型表征，并将颜色统计主要作为数据增强和分布诊断依据。
...
进一步的内容偏移分析如图 \ref{fig:content-shift} 所示。测试集的平均图像亮度为 0.6309，高于训练集的 0.6181，差值约为 \(+2.1\%\)；测试集平均对比度为 0.1125，低于训练集的 0.1253，差值约为 \(-10.2\%\)。p10--p90 区间显示两组样本在亮度和对比度上仍存在较大重叠，差异更适合解释为温和的统计偏移，而非训练集与测试集的整体分离。据此，训练阶段采用轻量 brightness/contrast/saturation jitter 提高颜色鲁棒性，并在伪标签自训练中使用低权重软标签，以降低教师模型预测噪声对监督信号的干扰。

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{train_test_content_shift_zoomed.png}
    \caption{训练集与测试集图像统计差异。纵轴采用局部范围以显示均值差异，误差线表示 p10--p90 区间；该图反映颜色和对比度存在温和偏移，同时两组样本统计区间仍有明显重叠。}
    \label{fig:content-shift}
\end{figure}

\subsection{数据预处理与数据增强}

本研究的关键设计变量是输入尺度校准。UNI2-h 等病理基础模型通常以 \(224\times224\) 病理图像块为输入，ViT 主干会将图像划分为固定 patch token 进行建模 \cite{dosovitskiy2021vit}。如果把 \(32\times32\) 单细胞图像直接拉伸到 \(224\times224\)，每条边会被放大 7 倍，细胞会占满整张画布。此时模型接收的是被过度放大的细胞结构，而不是病理视野中的中心局部目标；插值还会放大低分辨率纹理伪影，使 ViT token 捕获到的局部频率偏离预训练病理 tile 的统计特征。

最终方法采用 \texttt{CenterScalePad}：先将 \(32\times32\) 图像缩放到 \(64\times64\)，再以白色背景居中填充到 \(224\times224\)。如图 \ref{fig:scale-preprocess} 所示，这种处理保留了单细胞作为中心局部目标的语义，也减少了 \(32\rightarrow224\) 直接插值带来的纹理伪影。课堂汇报中的三视图对比进一步说明，在增加基础模型数量之前，输入尺度是否接近预训练视野是影响迁移效果的基础变量。
...
\begin{table}[H]
    ...
    \small
        ...
        外部评测模型 & full-250 single-refit，训练 11 epoch；对 61,881 张测试图像生成单模型预测 \\
        \bottomrule
    \end{tabular}
\end{table}
...
最终方法可以表述为一个两阶段的参数高效半监督迁移流程。第一阶段在 5-fold OOF 上比较尺度、微调方式、类别权重和伪标签策略；第二阶段固定 OOF 选出的配置，在 250 张有标签样本和筛选后的软伪标签上训练一个 single-refit 模型。该设计将输入尺度、可训练参数规模和无标签信息强度作为共同控制变量，在单一病理基础模型上完成任务适配。
...
本节从三个层次报告最终方案的实验结果。首先，训练曲线用于检查 full-refit 阶段的优化过程是否稳定；其次，5-fold OOF 指标用于评估在有标签训练集上的模型选择质量；最后，测试集预测分布与错误案例用于分析无标签大规模测试分布上的潜在风险。三类证据的统计含义不同：OOF 分数是可比较的本地验证结果，测试集预测分布属于无标签诊断证据，不作为隐藏测试性能估计。

图 \ref{fig:loss} 展示最终 single-refit 模型的训练损失。该阶段使用 250 张真实标签图像和 750 张均衡软伪标签图像，batch 中伪标签样本比例为 0.75；但由于伪标签损失被 \(\lambda_{\mathrm{pseudo}}=0.002\) 缩放，且伪标签有效权重和仅为 0.2357，优化过程仍以真实标签监督为主。损失从第 1 个 epoch 的 2.6464 降至第 4 个 epoch 的 0.6714，随后进入缓慢下降区间；第 8 个 epoch 为 0.4379，第 11 个 epoch 为 0.4297。该趋势表明 LoRA 参数和分类头的优化过程保持数值稳定；模型泛化能力则由 5-fold OOF 指标和课程平台隐藏测试反馈进行评价。
...
从类别维度看，模型在 \(\cls{0}\) 和 \(\cls{1}\) 上表现最稳定，recall 分别为 0.9600 和 0.9800，F1-score 分别为 0.9412 和 0.9515。这说明尺度校准后的 UNI2-h 表征可以较好地区分一部分形态边界清晰的类别。相对地，\(\cls{3}\) 和 \(\cls{4}\) 的 recall 均为 0.7600，F1-score 分别为 0.8085 和 0.7755，是最终模型的主要薄弱环节。在每折每类约 10 张验证样本的统计条件下，类别级指标的估计方差较大，因此本文主要将其用于分析相对类别难度和误差结构。

混淆矩阵进一步揭示了弱类错误的结构。最终 OOF 共 33 个错误样本，其中真实类别属于 \(\cls{2}\)、\(\cls{3}\)、\(\cls{4}\) 的错误为 30 个，占全部错误的 90.9\%。具体而言，\(\cls{3}\) 有 12 个样本被错分，其中 6 个被预测为 \(\cls{4}\)；\(\cls{4}\) 也有 12 个样本被错分，其中 5 个被预测为 \(\cls{2}\)，4 个被预测为 \(\cls{3}\)。这说明最终模型的主要困难不是所有类别均匀退化，而是集中在形态相近的中后段类别之间。对应的训练设计包括两部分：类别加权 hard CE 提高 \(\cls{3}\) 与 \(\cls{4}\) 在监督损失中的贡献，按预测类别均衡抽取的软伪标签则提高弱类在无标签扩展中的覆盖率。
...
\begin{figure}[H]
    ...
    \caption{最终模型与主要候选的 per-class recall 对比。scale64 pseudo 模型在整体均衡性上最好，残余误差主要集中于 \(\cls{3}\) 和 \(\cls{4}\)。}
    \label{fig:per-class-recall}
\end{figure}

\subsection{测试集预测分布诊断}

scale64 soft150 lam002 single-refit 模型在测试集上生成 61,881 条预测记录。由于测试集没有公开标签，表 \ref{tab:test-distribution} 和图 \ref{fig:test-distribution} 仅用于分析模型输出分布，不用于计算真实分类性能。预测分布呈明显不均衡：\(\cls{0}\) 占 45.08\%，\(\cls{3}\) 和 \(\cls{4}\) 分别占 21.85\% 和 19.31\%，而 \(\cls{1}\) 仅占 4.76\%。这一结果说明 balanced pseudo-label 训练并未把输出比例推向均匀分布，模型仍根据测试图像内容形成偏斜预测；在真实测试先验未知的情况下，\(\cls{0}\) 的高占比也提示 precision 风险。

\begin{table}[H]
    \centering
    \caption{外部评测预测文件的测试集预测分布}
...
\begin{figure}[H]
    ...
    \caption{外部评测预测分布。该图仅反映无标签测试集上的模型输出比例，不代表隐藏测试真实性能。}
...
综合来看，最终模型的错误可以分为两类：一类是高置信的形态近邻混淆，主要发生在 \(\cls{2}\)、\(\cls{3}\)、\(\cls{4}\) 之间；另一类是低置信的测试边界样本，表现为多个类别概率接近。前者表明阈值调整不足以解决稳定错分方向，后者说明隐藏测试分布可能包含训练集中覆盖不足的形态过渡样本。因而，伪标签扩展的有效性依赖于难类边界约束，例如类别条件阈值、更严格的边界样本筛选或轻量 adapter 对局部纹理的校正。
...
表 \ref{tab:route-comparison} 汇总了可追溯到正式 OOF/CV 记录、且与最终方法具有可比协议的关键探索节点。
...
\begin{table}[H]
    ...
    \begin{tabular}{L{2.6cm}L{6.8cm}rrL{6.5cm}}
        ...
        早期 fusion pseudo & 使用 Scheme02 fusion 教师模型的测试伪标签策略，伪标签集中于易类，\(\cls{3}\)/\(\cls{4}\) 没有伪标签。 & 0.7159 & 0.7160 & 不均衡伪标签没有超过监督融合基线，说明教师模型可靠性和类别覆盖都需要纳入控制。 \\
        ...
        UNI scale64 pseudo-250 & scale64 教师模型选取 250 张均衡伪标签，低权重伪标签损失。 & 0.8227 & 0.8234 & 当教师模型已经过尺度校准且伪标签保持类别覆盖时，无标签分布开始提供正收益。 \\
...
尺度消融用于检验输入视图是否是影响迁移效果的独立因素。实验保持 backbone、rsLoRA 注入位置、难类权重、fold 划分和训练策略不变，仅改变 \(32\times32\) 原图在 \(224\times224\) 画布中的有效显示尺度。由于 UNI2-h 使用 ViT-H/14 结构，patch size 为 14，因此输入尺度实际上决定了细胞横向覆盖的 patch 数量：full224 约覆盖 16 个 patch，scale32 约覆盖 2.3 个 patch，scale48 约覆盖 3.4 个 patch，scale64 约覆盖 4.6 个 patch，scale80 约覆盖 5.7 个 patch。该变量同时改变有效目标在 ViT token 网格中的空间跨度和上下文比例：full224 使细胞结构占据几乎全部输入视野，CenterScalePad 则将其呈现为画布中心的局部目标。
...
\begin{table}[H]
    ...
    \begin{tabular}{lrrrrL{6.0cm}}
        ...
        scale64 & 4.6 & 0.7500 & 0.7520 & 0.7510 & 略低于 scale48，但具备完整的教师模型、伪标签、OOF 与 full-refit 评估记录。 \\
...
尺度结果显示，直接 full resize 的 selection 为 0.7076，而 scale48 与 scale64 分别达到 0.7694 和 0.7510。由于这些实验只改变输入视图，不改变 backbone 或可训练参数规模，因此差异更合理地归因于输入尺度与预训练表征之间的匹配程度。scale32 只比 full224 高 0.0061，说明减少放大倍数本身并不足以充分表达细胞形态；scale80 低于 scale64，说明继续增大细胞占比会削弱局部目标假设。尺度收益呈非单调关系，关键在于使细胞在 ViT token 网格中保留适当的局部跨度。

外部评测方法选择 scale64 的依据是完整实验协议下的综合证据。scale48 在纯监督 OOF 中表现更好，但其伪标签与 adapter 变体没有超过 scale64 soft150 lam002；scale64 则在教师模型生成、balanced soft pseudo、5-fold OOF 和 single-refit 训练中使用一致的预处理与微调配置。因此，scale48 可视为有竞争力的尺度候选，而最终方法以 scale64 的完整可比结果为依据。

\subsection{伪标签与 adapter 消融}

伪标签实验的核心问题是：无标签测试图像能否提供额外分布信息，以及这种信息在何种条件下不会被教师模型预测噪声破坏。早期 fusion pseudo 结果给出了反例。Scheme02 fusion 教师模型的未均衡筛选策略只选择了 210 张伪标签，其中 \(\cls{0}=100\)、\(\cls{1}=80\)、\(\cls{2}=30\)，\(\cls{3}\) 和 \(\cls{4}\) 为 0；由该伪标签集训练得到的学生模型 selection 为 0.7160，低于监督融合基线的 0.7185。这说明在类别覆盖不足时，伪标签会强化容易类别的决策区域，并削弱 macro-F1 与 balanced accuracy 所强调的弱类贡献。

在尺度校准后的 UNI2-h 上，伪标签开始产生稳定收益。scale64 supervised 的 selection 为 0.7510；加入 250 张均衡伪标签后，selection 提升到 0.8234。随后将伪标签扩展为 soft100（每类 100 张，共 500 张）得到 0.8392；soft150（每类 150 张，共 750 张）在 \(\lambda=0.001\)、0.0015、0.002 下分别达到 0.8554、0.8596、0.8671。该递增序列表明，收益与尺度校准后的教师模型、按类别均衡抽样、soft label 以及低权重伪标签损失共同相关，而非单纯由无标签样本数量增加解释。
...
伪标签权重的结果也体现了少样本半监督训练的边界。对于 250 张伪标签，\(\lambda=0.001\) 达到 0.8234，而 \(\lambda=0.002\) 降至 0.8118；这说明当伪标签规模较小且仍包含教师模型预测噪声时，提高损失权重可能过早改变决策边界。对于 750 张 balanced soft pseudo，\(\lambda=0.002\) 反而高于 \(\lambda=0.001\)，说明在类别覆盖更完整、标签为 soft distribution、样本采用秩次权重后，模型可以承受略强的测试分布约束。因此，\(\lambda=0.002\) 的有效性应解释为 balanced soft150 设置下伪标签规模、软标签分布和秩次权重共同作用的结果。
...
这些结果表明，结构复杂度本身不是最终收益的主要来源。adapter 可以改善某些监督设置下的输入适配，特别是 scale48；但当伪标签教师模型、输入尺度和 full-refit 配置不一致时，adapter 的增益缺乏稳定证据。相比之下，scale64 虽然不是纯监督尺度消融中的最高项，但其教师模型生成、balanced soft pseudo 和 full-refit 训练均采用一致的预处理与 PEFT 配置，因而具有更完整的可比证据。由此可见，本项目的有效改进顺序是先校准输入尺度，再用保守 PEFT 适配基础模型，最后在类别均衡和低权重约束下引入软伪标签；单独增加 head 或 adapter 复杂度不足以替代这一组合。
...
外部评测结果来自 full-250 single-refit 模型对 61,881 张无标签测试图像的预测。该模型采用第 2 节所述的 scale64 预处理、UNI2-h backbone、rsLoRA 适配层和线性分类头。课程平台反馈结果如表 \ref{tab:hidden-score} 所示。本地 5-fold OOF 分数与隐藏测试分数分别来自训练集划分和外部测试集，前者用于候选模型比较，后者用于评价最终泛化表现。
...
隐藏测试反馈呈现出一个重要现象：Recall 为 0.7202，而 F1 为 0.5889。由于 F1 同时受到 precision 和 recall 影响，在 recall 相对较高而 F1 明显较低的情况下，与指标关系一致的解释是模型在隐藏测试集上覆盖了较多真实正例，同时 false positive 数量限制了精度项。平台未公开类别级混淆矩阵，因此尚无法定位主要误差类别；但本地证据提供了可解释的风险来源。首先，OOF 中 \(\cls{3}\) 与 \(\cls{4}\) 的 recall 均为 0.76，且 33 个 OOF 错误中有 30 个来自 \(\cls{2}\)、\(\cls{3}\)、\(\cls{4}\)，说明困难边界集中在形态相近类别之间。其次，最终预测分布并不均匀：\(\cls{0}\) 占 45.08\%，\(\cls{3}\) 与 \(\cls{4}\) 合计占 41.16\%，而 \(\cls{1}\) 和 \(\cls{2}\) 分别只有 4.76\% 和 9.00\%。这种分布表明 balanced soft pseudo 没有使输出比例趋于均匀；若高占比预测类包含较多边界样本，则宏平均 F1 会受到 precision 损失影响。

本地 OOF 与隐藏测试之间的落差还反映了少样本模型选择的统计边界。OOF 只有 250 张有标签图像，每折验证约 50 张；隐藏测试则包含 61,881 张图像，并且类别分布不均衡。小规模 OOF 可以用于比较候选方法，但难以完整覆盖测试集中低置信、形态过渡或染色分布变化的样本。测试预测置信度也支持这一判断：\(\cls{0}\) 的中位 Top-1 概率为 0.9319，而 \(\cls{2}\) 的中位 Top-1 概率仅为 0.7214、entropy 最高，为 0.8477。结合 OOF 中 \(\cls{2}\)-\(\cls{4}\) 的高置信错分案例，更谨慎的解释是类别边界在规模更大、类别先验未知的测试分布上产生了外推误差。

\subsection{大模型使用情况}

本项目使用公开预训练病理基础模型作为视觉表征来源，下游分类器训练未引入外部带标签图像数据。最终计分模型由 UNI2-h、rsLoRA 适配层和线性分类头构成；UNI2-h 的总参数量为 684,948,490，其中可训练参数为 3,554,314，占比约 0.52\%。因此，训练成本主要来自基础模型前向/反向计算与少量 LoRA 参数更新，推理成本对应单个 UNI2-h backbone 对 61,881 张测试图像的前向计算。

表 \ref{tab:model-license} 列出纳入比较范围的预训练模型及其用途。UNI2-h 是最终计分模型的 backbone；CONCH、Virchow2 与 H-optimus-0 用于冻结特征、单模型或融合消融，以判断多基础模型互补性是否优于尺度校准后的单模型。CONCH 属于视觉语言病理基础模型 \cite{lu2024conch}，其视觉语言能力仅用于比较性探索；最终方法采用纯视觉表征分类。外部预训练模型在本文中提供表征先验，下游监督信号限于给定的 250 张标注训练图像，无标签测试图像仅通过预测概率筛选形成伪标签。
...
\begin{table}[H]
    ...
    \begin{tabular}{llllp{4.8cm}}
        ...
        \midrule
        UNI2-h & \texttt{MahmoodLab/UNI2-h} & CC-BY-NC-ND-4.0 & 是 & 最终 backbone，rsLoRA 微调并用于外部评测预测。 \\
...
本项目的主要结论是：在每类仅 50 张训练样本的设置下，输入视图与预训练尺度的匹配程度会直接影响病理基础模型迁移效果。早期多 backbone fusion 将 selection 提升到约 0.7185，但仍使用普通 \(224\times224\) 输入视图；尺度校准后，单 UNI2-h supervised 已超过该融合基线。最终方法进一步结合 rsLoRA、难类加权和均衡软伪标签，在本地 5-fold OOF 上达到 macro-F1 0.8662、balanced accuracy 0.8680 和 selection 0.8671。该结果支持本文的核心判断：少样本分类中的主要收益来自输入尺度、参数高效适配和无标签样本筛选之间的协同设计，而非单纯增加 backbone 数量或分类头复杂度。

该方案仍存在三方面局限。第一，scale48 在纯监督尺度消融中高于 scale64，而 scale64 在教师模型生成、balanced soft pseudo、OOF 选择和 single-refit 训练中具有更完整的可比记录；因此本文关于最终方法的结论限定于当前已完成的完整实验协议。第二，隐藏测试 F1 低于本地 OOF，表明 250 张样本上的 5-fold OOF 对大规模不均衡测试集的泛化估计仍然偏乐观；本地 OOF 更适合作为候选模型比较证据，而非外部性能估计。第三，错误主要集中在 \(\cls{2}\)、\(\cls{3}\)、\(\cls{4}\) 的形态近邻边界，且隐藏反馈呈现 recall 高于 F1 的结构，说明最终预测具有较高类别覆盖，但 precision 仍受边界样本和伪标签噪声限制。总体而言，尺度校准和均衡软伪标签是本项目证据最充分的改进因素，其外部效度仍受隐藏测试反馈粒度和独立验证资源限制。
