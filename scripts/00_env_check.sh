#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# 00_env_check.sh — thin wrapper for `python -m ml_final.cli env-check`
#
# Usage:
#   bash scripts/00_env_check.sh [CLI args...]
#
# Notes:
...
if [ "$PYTHON_MAJOR" -lt 3 ] || { [ "$PYTHON_MAJOR" -eq 3 ] && [ "$PYTHON_MINOR" -lt 10 ]; }; then
    echo "ERROR: Python >= 3.10 required. Found $PYTHON_VERSION ($PYTHON_BIN)." >&2
...
# --- Run the CLI command ---
cd "$PROJECT_ROOT"
exec "$PYTHON_BIN" -m ml_final.cli env-check "$@"
